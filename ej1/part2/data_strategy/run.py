...
def split_done(raw_df, strategy, seed, split_spec):
    mask = (
...
def final_retrain_done(raw_df, strategy, seed):
    mask = (
        (raw_df["record_type"] == "split_summary")
        & (raw_df["experiment_type"] == EXPERIMENT_TYPE)
        & (raw_df["strategy"] == strategy)
...
def summarize_strategy(raw_df, strategy, run_cfg):
    seed_rows = raw_df[
        (raw_df["record_type"] == "split_summary")
        & (raw_df["experiment_type"] == EXPERIMENT_TYPE)
        & (raw_df["strategy"] == strategy)
    ...
    for seed in sorted(seed_rows["seed"].astype(int).unique()):
        seed_summary = aggregate_seed_summary(
            raw_df,
            {"experiment_type": EXPERIMENT_TYPE, "strategy": strategy, "seed": seed},
        )
    ...
    summary = {
        ...
        "learning_rate": float(run_cfg["training"]["learning_rate"]),
        "batch_size": int(run_cfg["training"]["batch_size"]),
...
def main():
    ...
    for strategy in ("S1", "S3"):
        ...
        for seed in seeds:
            if final_retrain_done(raw_df, strategy, seed):
                print(f"  {strategy} seed={seed}: ya completo, salteo.")
                continue

            for split_spec in split_specs:
                if split_done(raw_df, strategy, seed, split_spec):
                    continue
                ...
                split_key = f"{strategy}_seed{seed}_{split_spec['split_kind']}_{split_spec['split_id']}"
                ...
                curve_base = base_row(
                    ...
                    strategy=strategy, split_kind=split_spec["split_kind"], split_id=split_spec["split_id"],
                )
                train_base = base_row(
                    run_cfg, EXPERIMENT_TYPE, seed, len(train_idx), len(val_idx), len(test_idx),
                    strategy=strategy, split_kind=split_spec["split_kind"], split_id=split_spec["split_id"], subset="train",
                )
                val_base = base_row(
                    run_cfg, EXPERIMENT_TYPE, seed, len(train_idx), len(val_idx), len(test_idx),
                    strategy=strategy, split_kind=split_spec["split_kind"], split_id=split_spec["split_id"], subset="val",
                )
            ...
            final_key = f"{strategy}_seed{seed}_final_retrain"
            ...
            curve_base = base_row(
                ...
                strategy=strategy, split_kind="final_retrain", split_id=0,
            )
            train_base = base_row(
                run_cfg, EXPERIMENT_TYPE, seed, len(train_val_idx), 0, len(test_idx),
                strategy=strategy, split_kind="final_retrain", split_id=0, subset="train",
            )
            test_base = base_row(
                run_cfg, EXPERIMENT_TYPE, seed, len(train_val_idx), 0, len(test_idx),
                strategy=strategy, split_kind="final_retrain", split_id=0, subset="test",
            )
            ...
            if seed_summary is not None:
                print(
                    f"  {strategy} seed={seed}: val_f2={seed_summary['mean_val_f2']:.4f} "
                    f"test_f2={seed_summary['test_f2']:.4f} elapsed={seed_summary['elapsed_s_total']:.1f}s"
                )

        raw_df = load_raw_df(RAW_PATH)
        summary = summarize_strategy(raw_df, strategy, run_cfg)
        summaries = [row for row in summaries if row["strategy"] != strategy]
        if summary is not None:
            summaries.append(summary)
            print(
                f"  {STRATEGY_LABELS[strategy]}: val_f2={summary['mean_val_f2']:.4f} ± {summary['std_val_f2']:.4f} "
